...
const get_mailgun_api_key = () => {
  ...
  const apikeyFiles = fs.readdirSync(apikeyFolder);
  let apikey = "";
  for (const apikeyFile of apikeyFiles) {
    apikey += fs.readFileSync(apikeyFolder + "/" + apikeyFile, "utf8");
  }
  return apikey;
};

const get_mailgun_public_key = () => {
  const pubkeyFolder = KEYS_FOLDER + "/mg_pk";
  const pubkeyFiles = fs.readdirSync(pubkeyFolder);
  let pubkey = "";
  for (const pubkeyFile of pubkeyFiles) {
    pubkey += fs.readFileSync(pubkeyFolder + "/" + pubkeyFile, "utf8");
  }
  return pubkey;
};
...
const mg = mailgun.client({
  username: "mhacks.org",
  key: process.env.MAILGUN_API_KEY || get_mailgun_api_key(),
  public_key: process.env.MAILGUN_PUBLIC_KEY || get_mailgun_public_key(),
});
