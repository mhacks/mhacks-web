...
router.post('/application', uploadHelper.fields([{ name: 'resume' }]), function(req, res) {
    var updateable_fields = MentorApplication.getUpdateableFields(req.groups);
    var fields = {};
    ...
    MentorApplication.find().byUser(req.user).then(application => {
        if (application) {
...
router.get('/application', function(req, res) {
    MentorApplication.find({}, '-_id -__v')
        .byUser(req.user)
        .then(application => {
            res.send({
                status: true,
